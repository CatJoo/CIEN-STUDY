...
public class Player_manager : MonoBehaviour
{
    ...
    [SerializeField] 
    UI_Manager _uimanager;  //UI manager 하청으로 넣어주기(이름 스크립트 이름이랑 같게 주의!!!!)
    public SceneChanger _changeScene;  //scene changer 하청으로 넣어주기(이름 스크립트 이름이랑 같게 주의!!!!)

    [SerializeField]
    UITweener tweener;   //player manager에서 ui tweener 쓸거라서 써주기
    ...
    void Update() //매 순간마다 체크
    {
        ...
        //uimanager.SetStemina(_stemina);   //stemina 값이 변할 때 ui에도 적용이 되도록
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)  //부딪힌 애를 return하는 함수
    {
        ...
        if (hit.gameObject.name == "fire")  //부딪힌 애의 이름이 fire이면 heath -1 
        {
            ...
            {
                _health -= 1;
                tweener.ShakeIt();  //체력이 깎일 때 UI에 tween 적용해서 흔들리게
            }
            else
            {
                _isAlive = false;
                ...
                _changeScene.ChangeSceneByName_asdf("MainMenuScene");  //죽었을 때 change scene 하도록
